...
/**
 * Class that represents [DAOVideoCache] of the application.
 * It uses a folder instead of a real database to store videos an indexes,
 */
class DAOVideoCache(val uploadDir: File) {
    /**
     ...
     */
    private fun listAll(): Sequence<Video>? = allIds?.asSequence()?.mapNotNull { videoById(it) }
